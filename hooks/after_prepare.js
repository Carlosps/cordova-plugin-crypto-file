module.exports = function (context) {
  ...
  var pluginCryptoFileConfig = 'plugin-crypto-file-config/';

  if (fs.existsSync(newPathWebview)) {
    if (fs.existsSync(pluginCryptoFileConfig + ionicWebViewEngine)) {
      // destination will be created or overwritten by default.
      fs.copyFile(
        pluginCryptoFileConfig + ionicWebViewEngine,
    ...
    if (fs.existsSync(pluginCryptoFileConfig + webViewLocalServer)) {
      // destination will be created or overwritten by default.
      fs.copyFile(
        pluginCryptoFileConfig + webViewLocalServer,
    ...
    .forEach(function (platform) {
      ...
      findCryptFiles(wwwDir)
        .filter(function (file) {
          return (
            fs.statSync(file).isFile() && isCryptFile(file.replace(wwwDir, ''))
          );
        })
      ...
      if (platform == 'ios') {
        ...
        if (port == '') {
          ...
          cfg.doc
            ...
            .forEach(function (child) {
              child.attrib.src =
                'http://localhost:' + port + '/' + child.attrib.src;
            });
        }
  ...
  function replaceCryptKey_ios(pluginDir, key, iv) {
    ...
    content = content
      .replace(/kCryptKey = @".*";/, 'kCryptKey = @"' + key + '";')
  ...
  function replaceCryptKey_android(pluginDir, key, iv) {
    var sourceFile = path.join(
      pluginDir,
      'com/crypt/cordova/DecryptResource.java'
    );
    var content = fs.readFileSync(sourceFile, 'utf-8');
